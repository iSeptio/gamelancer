...
class TestTicketAPI(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            'user@random.com', email='user@random.com', password=self.PASSWORD)
        ...
        self.user_client.login(
            username='user@random.com', password=self.PASSWORD)
        self.anon_client = APIClient()
        self.admin_client = APIClient()
        self.admin_client.login(
            username='admin@business.com', password=self.PASSWORD)

    def test_create_user_ticket(self):
        url = reverse('ticket-create')
        response = self.user_client.post(
            url, self.authenticated_ticket, format='json')
    ...
    def test_tickets_meta_retrieve_return_meta_as_downloadable_json(self):
        ...
        self.assertEqual(json.loads(response.content),
                         self.anon_ticket.get('meta'))
        self.assertEqual(content_type, ('Content-Type', 'application/json'))
        self.assertEqual(content_disposition, ('Content-Disposition',
                                               'attachment; filename={}.json'.format(attachment_name)))
